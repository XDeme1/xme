...
namespace xme::math {
template<arithmetic_c, std::size_t>
struct Vector;

//! Linearly interpolates `v1` and `v2`.
//! Uniform operation.
template<floating_point_c T, arithmetic_c U, std::size_t N>
[[nodiscard]]
XME_INLINE constexpr auto lerp(const Vector<T, N>& v1, const Vector<T, N>& v2, U percent) {
    if constexpr(same_as_c<std::remove_cv_t<U>, bool>) {
...
template<typename T>
    requires signed_integral_c<T> || std::numeric_limits<T>::is_iec559
[[nodiscard]]
