...
public:
    ...
    template<typename U>
        requires CSquareMatrix<Cols, Rows>
    ...
    constexpr Matrix(U s) noexcept : m_Data({{s, 0}, {0, s}}) {}

    template<typename U>
        requires CSquareMatrix<Cols, Rows> && (Cols == 3)
    constexpr Matrix(U s) noexcept : m_Data({{s, 0, 0}, {0, s, 0}, {0, 0, s}}) {}

    template<typename U>
        requires CSquareMatrix<Cols, Rows> && (Cols == 4)
    constexpr Matrix(U s) noexcept
        : m_Data({{s, 0, 0, 0}, {0, s, 0, 0}, {0, 0, s, 0}, {0, 0, 0, s}}) {}
    ...
    template<typename...Args>
        requires(Rows == 2)
    constexpr Matrix(const Vector<Args, 2>&...args) noexcept 
        : m_Data({args...}) {}

    template<typename...Args>
        requires(Rows == 3)
    constexpr Matrix(const Vector<Args, 3>&...args) noexcept 
        : m_Data({args...}) {}
